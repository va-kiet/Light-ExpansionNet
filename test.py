...
def evaluate_model(ddp_model,
    ...
    if rank == 1 and verbose:
        ...
        score_results = coco_eval.evaluate(bleu=True, rouge=True, cider=True, spice=False, meteor=True, verbose=False)
...
def test(rank, world_size,
    ...
    if is_end_to_end:
        ...
        model = LightExpansionNet(d_model=model_args.model_dim, N_enc=model_args.N_enc,
                                N_dec=model_args.N_dec, num_heads=8, ff=2048,
                                num_exp_enc_list=[32, 64, 128, 256, 512],
                                num_exp_dec=16,
                                output_word2idx=coco_dataset.caption_word2idx_dict,
    ...
    if rank == 1 and show_predictions:
