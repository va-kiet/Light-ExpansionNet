...
class End_LightExpansionNet(CaptioningModel):
    def __init__(self,
                 ...
                 swin_use_checkpoint, swin_pretrained_window_sizes,
        ...
        super(End_LightExpansionNet, self).__init__()
        ...
        self.swin_transf = SwinTransformer(
                 ...
                 use_checkpoint=swin_use_checkpoint, pretrained_window_sizes=swin_pretrained_window_sizes)
        ...
        self.encoders = nn.ModuleList([EncoderLayer(d_model, num_heads, ff, num_exp_enc_list, drop_args.enc) for _ in range(N_enc)])
    ...
    def forward_enc(self, enc_input, enc_input_num_pads):
        ...
        pad_mask_attn = create_pad_mask(mask_size=(enc_input.size(0), enc_input.size(1), enc_input.size(1)),
                                   pad_row=[0] * enc_input.size(0),
                                   pad_column=enc_input_num_pads,
                                   rank=self.rank)
        x_list = []
        for i in range(self.N_enc):
            x = self.encoders[i](x=x, n_indexes=pos_x, mask=pad_mask, attn_mask=pad_mask_attn)
            x_list.append(x)
